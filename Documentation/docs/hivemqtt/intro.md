...
`HiveMQtt` is the NuGet package, and `HiveMQClient` is the primary high-level client type most applications use.
...
```
Then check out the [Quickstart Guide](/docs/hivemqtt/quickstart) to begin.
:::

:::info Looking for Sparkplug?
The Sparkplug client is documented separately. Start with [HiveMQtt.Sparkplug](/docs/sparkplug/intro) for Host Application and Edge Node guides.
:::
...
- **SecureString Support**: [Secure credential handling](/docs/hivemqtt/security#securestring-for-credentials) to prevent password exposure in memory
- **X.509 Client Certificates**: [Full support](/docs/hivemqtt/how-to/client-certificates) for certificate-based authentication
- **Username/Password Auth**: Simple credential-based authentication
- **[Security Best Practices](/docs/hivemqtt/security)**: Comprehensive guide for secure implementations

### Developer Experience
- **Extensive Event System**: [Hook into all operations](/docs/hivemqtt/events) from connection to packet level
- **Detailed Logging**: [TRACE level logging](/docs/hivemqtt/how-to/debug) for debugging
- **Builder Pattern APIs**: Fluent interfaces for easy configuration
- **Smart Defaults**: Works out of the box with sensible defaults

### Performance
- **Optimized & Benchmarked**: See [benchmark results](/docs/hivemqtt/benchmarks)
- **RawClient (Beta)**: [Low-level client](/docs/rawclient/intro) for maximum performance scenarios
...
| Client | Best For |
|--------|----------|
| [`HiveMQClient`](/docs/hivemqtt/hivemqclient) | Most applications - full features including subscription tracking, per-subscription callbacks, and automatic reconnect |
| [`RawClient`](/docs/rawclient/intro) | Performance-critical scenarios - minimal overhead, no subscription tracking |

For Sparkplug B 3.0 workflows, use the dedicated [Sparkplug Client docs](/docs/sparkplug/intro).
