...
**Description:** When enabled, the client does not send PubAck or PubRec until the application calls `AckAsync` on the client. Unacked messages consume Receive Maximum slots. Use this when you need to process or persist messages before the broker is told they were received. See the [Manual Acknowledgement](/docs/hivemqtt/how-to/manual-ack) how-to for details.
