...
public class WebSocketTransport : BaseTransport, IDisposable
{
    ...
    private bool ValidateWebSocketServerCertificate(
        object sender,
        ...
        System.Net.Security.SslPolicyErrors sslPolicyErrors)
    {
        ...
        this.Logger.Warn("Broker TLS Certificate error for WebSocket: {0}", sslPolicyErrors);

        // Log additional certificate details for debugging
        if (certificate != null)
        {
            this.Logger.Debug("WebSocket Certificate Subject: {0}", certificate.Subject);
            this.Logger.Debug("WebSocket Certificate Issuer: {0}", certificate.Issuer);
            this.Logger.Debug("WebSocket Certificate Serial Number: {0}", certificate.GetSerialNumberString());
        }
        ...
        if (chain != null)
        {
            ...
            this.Logger.Debug("WebSocket Certificate chain validation status: {0}", chainStatus);
        }
    ...
    private void ConfigureWebSocketOptions()
    {
        ...
        if (this.Options.WebSocketKeepAliveInterval.HasValue)
        {
            ...
            this.Logger.Trace($"WebSocket keep-alive interval set to {this.Options.WebSocketKeepAliveInterval.Value}");
        }
        ...
        if (this.Options.WebSocketRequestHeaders != null && this.Options.WebSocketRequestHeaders.Count > 0)
        {
            ...
            this.Logger.Trace($"Added {this.Options.WebSocketRequestHeaders.Count} custom header(s) for WebSocket connection");
        }
        ...
        if (this.Options.WebSocketProxy != null)
        {
            ...
            this.Logger.Trace($"WebSocket proxy configured: {this.Options.WebSocketProxy}");
        }
    }
    ...
    private void ConfigureTlsOptions()
    {
        ...
        if (this.Options.ClientCertificates != null && this.Options.ClientCertificates.Count > 0)
        {
            ...
            this.Logger.Trace($"Added {this.Options.ClientCertificates.Count} client certificate(s) for WebSocket connection");
        }

        // Configure certificate validation callback
        if (this.Options.AllowInvalidBrokerCertificates)
        {
            this.Logger.Trace("Allowing invalid broker certificates for WebSocket connection");
...
#pragma warning restore CA5359
        ...
        else
        {
            ...
            this.Socket.Options.RemoteCertificateValidationCallback = this.ValidateWebSocketServerCertificate;
        }
    }
    ...
    public override async Task<bool> ConnectAsync(CancellationToken cancellationToken = default)
    {
        ...
        catch (WebSocketException ex)
        {
            ...
            this.Logger.Debug(ex, "Failed to connect to the WebSocket server");
            return false;
        }
    ...
    public override async Task<bool> CloseAsync(bool? shutdownPipeline = true, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                this.Logger.Debug("WebSocket is already aborted");
            }
            else if (this.Socket.State == WebSocketState.Closed)
            {
                this.Logger.Debug("WebSocket is already closed");
            }
            else
            {
                this.Logger.Debug($"WebSocket is in state: {this.Socket.State}");
            }
        }
        catch (WebSocketException ex)
        {
            // WebSocket may have been closed by the server or already in an invalid state
            this.Logger.Warn(ex, "Error closing WebSocket connection");
        ...
        catch (ObjectDisposedException ex)
        {
            ...
            this.Logger.Debug(ex, "WebSocket has already been disposed");
        }
        catch (InvalidOperationException ex)
        {
            // WebSocket may already be closed or in a closing state
            this.Logger.Debug(ex, "WebSocket is already closed or closing");
        }
        catch (OperationCanceledException ex)
        {
            // Operation was cancelled
            this.Logger.Debug(ex, "Close operation was cancelled");
            return false;
        }
    ...
    public override async Task<bool> WriteAsync(byte[] buffer, CancellationToken cancellationToken = default)
    {
        ...
        catch (OperationCanceledException)
        {
            ...
            this.Logger.Debug("Write operation was cancelled while waiting for semaphore");
            return false;
        }
        ...
        try
        {
            ...
            {
                this.Logger.Debug($"WebSocket is not in a writable state: {this.Socket.State}");
                return false;
            }
        ...
        catch (WebSocketException ex)
        {
            ...
            this.Logger.Debug(ex, "Failed to write to the WebSocket server");
            return false;
        }
        catch (OperationCanceledException ex)
        {
    ...
    public override async Task<TransportReadResult> ReadAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            this.Logger.Debug($"WebSocket is not in a readable state: {this.Socket.State}");
        ...
        try
        {
            ...
            {
                this.Logger.Debug($"WebSocket received close message: {result.CloseStatus}");
                return new TransportReadResult(true);
            }
            ...
            {
                this.Logger.Debug($"WebSocket connection closed during read: {result.CloseStatus}");
                return new TransportReadResult(true);
            }
            ...
            if (result.EndOfMessage && result.MessageType is WebSocketMessageType.Binary)
            {
                ...
                if (result.Count > 0)
                {
                    ...
                    this.Logger.Trace($"Received {result.Count} bytes (single read)");
            ...
            using (var ms = new MemoryStream())
            {
                ...
                this.Logger.Trace($"Received {ms.Length} bytes (fragmented)");
                ...
                else
                {
                    ...
                    this.Logger.Warn($"Received unexpected WebSocket message type: {result.MessageType}");
                    return new TransportReadResult(true);
                }
            }
        }
        catch (WebSocketException ex)
        {
            ...
            this.Logger.Debug(ex, "Failed to read from the WebSocket server");
            return new TransportReadResult(true);
        }
        catch (OperationCanceledException ex)
        {
            this.Logger.Debug(ex, "Read operation was canceled");
            return new TransportReadResult(true);
        }
        catch (InvalidOperationException ex)
        {
            // Socket may have been closed or aborted
            this.Logger.Debug(ex, "WebSocket operation is invalid - socket may be closed");
            return new TransportReadResult(true);
        }
        finally
        {
    ...
    {
        this.Logger.Trace("Disposing WebSocketTransport");
...
#pragma warning restore VSTHRD002
                    ...
                    {
                        this.Logger.Warn($"Error closing WebSocket: {ex.Message}");
                    }
                    finally
                    {
                        this.Socket.Dispose();
