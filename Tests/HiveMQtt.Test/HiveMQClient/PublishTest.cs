...
public class PublishTest
{
    ...
    public async Task ThreeNodeQoS0ChainedPublishesAsync()
    {
        ...
        Assert.Equal(1000, client2MessageCount);
        Assert.Equal(1000, client3MessageCount);
    ...
    public async Task ThreeNodeQoS1ChainedPublishesAsync()
    {
        ...
        async void Client2MessageHandler(object? sender, OnMessageReceivedEventArgs eventArgs)
        {
            client2MessageCount++;
            var client = sender as HiveMQClient;
            var publishResult = await client.PublishAsync("HMQ/SecondTopic", eventArgs.PublishMessage.PayloadAsString, QualityOfService.AtLeastOnceDelivery).ConfigureAwait(true);
            Assert.NotNull(publishResult);
